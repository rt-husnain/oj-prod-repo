    ...
    if (isset($_REQUEST['type']) && $_REQUEST['type'] == '2') {
		...
		foreach ($dataSet as $index => $attributes) {
			$title = getTitle($attributes);
			 if ($attributes["event_group_c"] == 'MO') {
				$type = 'MO';
				if ($attributes['participant_type'] == 'Delegate') {
                                        $data['participants_mo'] [] =  '<li><span></span>' . $title . '</li>';
...
ORDER BY account_name";
    ...
    while ($sponosors_row = $db->fetchByAssoc($sponosors_res)) {
        ...
        WHERE
            oj_events.deleted = 0
                AND atc.event_status_c IN ('YC' , 'Y', 'HP')
          ...
          $sub_response = $db->query($subquery);
            while($sub_row = $db->fetchByAssoc($sub_response)){
                if($sub_row['participant_type'] == 'Sponsor' || $sub_row['participant_type'] == 'Expert' || $sub_row['participant_type'] == 'Delegate'){
...
if ($type == 'MO') {
    ...
    if (!empty($data['participants_mo'])) {
		sort($data['participants_mo']);
        $test .= '<tr><td colspan="2" style="height:10px"></td></tr>';
        $test .= '<tr><td width="38"></td><td width="175%"><span style="font-weight:bold;font-size:12; color:#b99d51;height:10px" width="175%">PARTICIPANTS</span><ul>' . implode("",$data['participants_mo']) . '</ul></td></tr>';
        if ($_REQUEST['type'] == '3') {
            $test .= '<tr><td width="38"> </td><td style="color:#b99d51"  width="175%"><h4>Total :' . sizeof($data["participants_mo"]) . '</h4></td></tr>';
        }
    }
